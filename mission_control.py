...
class Task:
    ...
    def priority_score(self) -> float:
        """Calculate priority score: IMPACT > URGENCY > EFFORT (lower is better for sorting)"""
...
class MissionControl:
    ...
    async def scan_github_issues(self):
        ...
        if not self.github_token:
            print("    ⚠️ GITHUB_TOKEN not set, skipping GitHub scan")
            return
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                for issue in issues:
                    if issue.get("state") == "OPEN":
                        ...
                        is_blocker = any(w in l for l in labels for w in ["blocker", "bug", "critical"])
            ...
            print(f"    ⚠️ Could not scan GitHub issues: {e}")
    
    async def scan_workflow_runs(self):
        """Scan GitHub Actions workflow runs"""
        print("  ⚙️ Scanning workflow runs...")
        ...
        try:
            ...
            print(f"    ⚠️ Could not scan workflows: {e}")
    ...
    async def scan_task_files(self):
        ...
        if not yaml:
            print("    ⚠️ PyYAML not installed, skipping yaml task files")
            return
        ...
        if tasks_dir.exists():
            for task_file in tasks_dir.glob("*.yaml"):
                ...
                try:
                    ...
                    print(f"    ⚠️ Could not read {task_file}: {e}")
                ...
                for line in result.stdout.strip().split('\n'):
                    if line.strip():
                        try:
                            ...
                            # Check state. Field names depend on Docker version, usually 'State' or 'Status'
                            state = container.get("State", "").lower()
                            status = container.get("Status", "").lower()
                            
                            if "exited" in state or "exited" in status:
                                name = container.get("Names", "unknown")
                                task = Task(
                                    id=f"DOCKER-{name[:20]}",
                                    title=f"Restart Docker: {name}",
                                    source="Docker",
                                    category=TaskCategory.FIX,
                                    priority=Priority.MEDIUM,
                                    impact=5,
    ...
    def prioritize_tasks(self):
        """Prioritize all tasks by SCORE (Lower is better)"""
    ...
    def get_top_levers(self, limit: int = 10) -> List[Task]:
        ...
        # Sort by impact descending (10 is high)
    ...
    def generate_dashboard(self) -> str:
        ...
        if levers:
            ...
            dashboard += "*No high-impact tasks* ⚠️\n"
        dashboard += "\n"
    ...
    def generate_action_list(self, limit: int = 7) -> str:
        ...
        for i, task in enumerate(action_tasks, 1):
            icon = "🔥" if task.blocker else "⚡" if task.impact >= 8 else "✓"
            action_list += f"{i}. {icon} **{task.title}** ({task.category.value})\n"
            action_list += f"   - Source: {task.source}\n"
            action_list += f"   - Priority: Impact={task.impact}, Urgency={task.urgency}, Effort={task.effort}\n"
            if task.blocker:
                action_list += f"   - ⚠️ BLOCKER: {task.blocker_reason}\n"
            action_list += "\n"
