...
from antigravity.config import REPORTS_DIR, ISSUES_FILE, KANBAN_FILE
...
def cluster_issues(issues):
    ...
    for key, items in sorted(groups.items(), key=lambda x: CLUSTERS.get(x[0],{}).get("priority",9)):
        ...
        tasks.append({"id":f"TASK-{tid:03d}","cluster":key,"label":cd["label"],
            "owner_agent":cd["owner"],"priority":cd["priority"],"status":"backlog",
            "issue_count":len(items),"files_affected":files,
            "acceptance_criteria":[f"All {len(items)} issues resolved","Tests pass","No regressions"],
            "issues":items[:50]})
        tid += 1
    return {"generated_at":time.strftime("%Y-%m-%dT%H:%M:%S"),"total_tasks":len(tasks),
            "total_issues":len(issues),"tasks":tasks}

def generate_kanban(data):
    lines = ["# 🔥 ANTIGRAVITY – Issues Kanban",f"_Generated: {data['generated_at']}_ | Issues: {data['total_issues']}","","## 📋 Backlog",""]
    for t in data["tasks"]:
        icon = {1:"🔴",2:"🟡",3:"🔵",4:"⚪"}.get(t["priority"],"⚫")
        lines += [f"### {icon} {t['id']}: {t['label']}",f"- Owner: `{t['owner_agent'].upper()}` | Issues: {t['issue_count']}",
